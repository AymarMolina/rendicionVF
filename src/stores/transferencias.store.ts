...
export const useTransferenciasStore = defineStore('transferencias', () => {
  ...
  // Nuevo: transferencias con deadline crítico o vencido (para mostrar alertas)
  const conAlertaDeadline = computed(() =>
    transferencias.value
      .filter(t => t.deadline && t.deadline.estado !== 'ok' && t.deadline.estado !== 'sin_fecha'
                && t.estado !== 'aprobada')
      .sort((a, b) => (a.deadline.dias_restantes ?? 0) - (b.deadline.dias_restantes ?? 0))
  )
