    ...
    <div class="card mb">
      ...
      <div class="card-body">
        ...
        <template v-else>
          ...
          <div v-if="selectedFile && !previewUrl" class="file-ok">
            ...
            <tr v-for="g in gastosActivos" :key="g.id">
              ...
              <td>
                <div style="display:flex;gap:6px">
                  <button class="icon-btn edit" @click="editarGasto(g)" title="Editar"><Pencil class="btn-ico" /></button>
                  <button class="icon-btn" @click="eliminarGasto(g.id)" title="Eliminar" :disabled="g.estado === 'aprobado'">
      ...
      <div class="modal-inner">
        ...
        <div class="form-row r3" style="margin-bottom:14px">
          ...
          <div class="form-group">
            ...
            <select v-model="mov.modulo_id" class="form-input">
        ...
        <div class="modal-footer">
          ...
          <button class="btn primary" @click="agregarMov"><Plus class="btn-ico" /> Confirmar y Registrar</button>
        </div>
      </div>
    </BaseModal>
  </div>
...
import { Plus, Trash2, Upload, CheckCircle, Info, X, Paperclip, Pencil, MapPin } from 'lucide-vue-next'
...
const form = reactive({
  ...
  mov_punto_partida: '', mov_punto_llegada: '', mov_modulo_id: 0, mov_motivo: ''
})

const dj  = reactive({ fecha: '', rubro: 'alimentos', detalle: '', monto: 0, nombre_proveedor: '', dni_proveedor: '', lugar: '' })
const mov = reactive({ partida: '', llegada: '', modulo_id: 0, motivo: '' })
...
async function agregarGasto() {
  ...
  if (form.rubro === 'transporte' && !form.mov_punto_partida) {
    movilFlow.value = 'ruc'; mov.modulo_id = auth.moduloPrimario?.modulo_id ?? 0
    showMovilidad.value = true; return
  }
  if (!form.tieneRuc && !form.dj_nombre_proveedor) {
    showDJ.value = true
    toast.warning('Declaración Jurada requerida', 'Complete los datos del proveedor'); return
  }
  await guardarGasto()
}
...
async function guardarGasto() {
  try {
    ...
    await store.cargarGastos(selectedTransfId.value)
    cancelarEdicion()
    presupInfo.value = null; montoError.value = false; movilFlow.value = null
  } catch (e: any) {
    toast.error('Error', e.message)
  }
}
...
function cerrarDJ() {
  showDJ.value = false
  if (!form.dj_nombre_proveedor) form.tieneRuc = true
}

async function agregarDJ() {
  if (!dj.fecha || !dj.detalle || !dj.monto || !dj.nombre_proveedor) {
    toast.warning('Campos incompletos', 'Complete fecha, proveedor, detalle y monto'); return
  }
  if (dj.rubro === 'transporte' && !form.mov_punto_partida) {
    movilFlow.value = 'dj'; showMovilidad.value = true; return
  }
  await guardarDJ()
}
...
async function guardarDJ() {
  try {
    await store.registrarGasto({
      ...
      dj_lugar:            dj.lugar,
      ...(dj.rubro === 'transporte' && {
        mov_punto_partida:  form.mov_punto_partida,
        mov_punto_llegada:  form.mov_punto_llegada,
        mov_institucion_id: form.mov_modulo_id,
        mov_motivo:         form.mov_motivo,
      })
    }, null)
    await store.cargarGastos(selectedTransfId.value)
    dj.detalle = ''; dj.monto = 0; dj.nombre_proveedor = ''; dj.dni_proveedor = ''; dj.lugar = ''
    form.mov_punto_partida = ''; form.mov_punto_llegada = ''; movilFlow.value = null
...
function abrirEditarMovilidad() {
  ...
  mov.modulo_id = form.mov_modulo_id || auth.moduloPrimario?.modulo_id || 0
  mov.motivo    = form.mov_motivo
  movilFlow.value = 'ruc'; showMovilidad.value = true
}

function cerrarMovilidad() {
  showMovilidad.value = false; movilFlow.value = null
  if (!form.mov_punto_partida) form.rubro = '' as RubroGasto
  mov.partida = ''; mov.llegada = ''; mov.motivo = ''
}

async function agregarMov() {
  if (!mov.partida || !mov.llegada || !mov.modulo_id) {
    toast.warning('Campos incompletos', 'Complete partida, llegada y módulo'); return
  }
  form.mov_punto_partida = mov.partida
  form.mov_punto_llegada = mov.llegada
  form.mov_modulo_id     = mov.modulo_id
  form.mov_motivo        = mov.motivo
  showMovilidad.value = false
  if (movilFlow.value === 'ruc') await guardarGasto()
  else if (movilFlow.value === 'dj') await guardarDJ()
  mov.partida = ''; mov.llegada = ''; mov.motivo = ''
}
...
function cancelarEdicion() {
  editandoId.value = null; form.concepto = ''; form.monto = 0; form.nComprobante = ''
  form.rubro = '' as RubroGasto; form.tieneRuc = true
  intentoGuardar.value = false; limpiarArchivo()
}

// ── Watchers ───────────────────────────────────────────────────────────────
watch(nivelFiltro, () => {
  // Si la transferencia seleccionada ya no está en el filtro, seleccionar la primera disponible
...
watch(() => form.tieneRuc, (val) => {
  ...
  else { form.dj_nombre_proveedor = ''; form.dj_descripcion = ''; form.dj_dni_proveedor = ''; form.dj_lugar = '' }
})

watch([() => form.rubro, () => form.monto], async ([rubro, monto]) => {
  if (!rubro || !monto || !selectedTransfId.value) { presupInfo.value = null; montoError.value = false; return }
  const info = await store.checkPresupuesto(selectedTransfId.value, rubro, monto)
  presupInfo.value = info; montoError.value = info.excede
})

onMounted(async () => {
  form.fecha = new Date().toISOString().split('T')[0]
  dj.fecha   = form.fecha
  // Módulo por defecto para movilidad
  mov.modulo_id = auth.moduloPrimario?.modulo_id ?? 0
...
.transf-select {
  ...
  border:1.5px solid #d4dae8; border-radius:8px;
  font-size:13px; outline:none; color:#1a2340;
}
...
.pill {
  ...
  border:1.5px solid #d4dae8; background:#f9fafb;
  font-size:12px; font-weight:600; color:#6b7597;
...
.factura-toggle { display:inline-flex; align-items:center; gap:10px; font-size:13px; font-weight:600; color:#1a2340; cursor:pointer; background:#f0f4ff; border:1.5px solid #d4dae8; border-radius:8px; padding:8px 14px; transition:all .2s; }
.factura-toggle.active { background:#dbeafe; border-color:#2c4fd4; color:#1a2f6e; }
.toggle-track { width:34px; height:18px; background:#d4dae8; border-radius:9px; position:relative; transition:background .2s; flex-shrink:0; }
