  ...
  <div class="page">
    ...
    <div class="card">
      <div class="card-header">
        ...
        <!-- Toolbar: pills de nivel + búsqueda -->
        <div class="toolbar-right">
          <!-- Pills de nivel — solo muestra los niveles que tiene el tesorero -->
          <div class="nivel-pills">
            <button
              class="pill"
              :class="{ active: nivelFiltro === '' }"
              @click="nivelFiltro = ''"
            >
              Todos
            </button>
...
const q           = ref('')
const nivelFiltro = ref('')  // '' = todos, 'inicial' | 'primaria' | 'secundaria'
...
// ── Niveles disponibles para las pills (deducidos de los datos) ─────────────
...
// Dado un nivel, devuelve el código modular correspondiente
function codigoModularDe(nivel: string) {
  const t = store.transferencias.find(t => t.nivel === nivel)
  return t?.codigo_modular ?? ''
}
...
const grupos = computed(() => {
  ...
  // Filtro por nivel (pill)
  if (nivelFiltro.value) {
    lista = lista.filter(t => t.nivel === nivelFiltro.value)
  }

  // Filtro por texto (código / estado)
  if (term) {
  ...
  for (const t of lista) {
    if (!mapa.has(t.asignacion_id)) {
      mapa.set(t.asignacion_id, {
        asignacion_id:      t.asignacion_id,
        nivel:              t.nivel,
        codigo_modular:     t.codigo_modular,
        ciclo:              t.ciclo,
        monto_total:        t.monto_total,
        num_transferencias: t.num_transferencias,
        transferencias:     []
      })
    }
  ...
  for (const g of mapa.values()) {
    g.transferencias.sort((a: any, b: any) => a.numero - b.numero)
  }
  ...
  if (donutRef.value) {
    ...
    donutChart = new Chart(donutRef.value, {
      ...
      data: {
        ...
        datasets: [{ data: rubros.map(r => r.presupuesto_rubro), backgroundColor: COLORS.slice(0, rubros.length), borderWidth: 2, borderColor: '#fff', hoverOffset: 6 }]
      },
        ...
        tooltip: {
          callbacks: {
            title: items => {
              ...
              return `${t.codigo} · Transf. ${t.numero}/${t.num_transferencias} · ${nivelLabel(t.nivel)}`
            },
...
watch(() => store.transferencias.length, async n => { if (n > 0) { await nextTick(); initMainChart() } })
</script>
...
/* Pills con color de nivel cuando están activas */
...
td.empty { text-align:center; color:#6b7597; padding:24px; colspan:9; }

/* Grupo header */
.grupo-header td { background:#f7f8fc; padding:7px 16px; border-top:2px solid #d4dae8; border-bottom:1px solid #e5e8f0; }
.grupo-info { display:flex; align-items:center; gap:10px; flex-wrap:wrap; }
.grupo-ciclo { font-size:12px; color:#6b7597; }
.grupo-total { font-size:11.5px; color:#9ba6c0; margin-left:auto; }
